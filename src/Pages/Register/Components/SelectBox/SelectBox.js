...
function SelectBox({ Data }) {
	...
	const [onSelectVal, setOnSelectVal] = useState('선택해 주세요')

	const onClickSelect = data => {
		setOnSelectVal(data.target.innerText)
	}

	return (
		<S.Wrapper onClick={() => setIsView(!isView)}>
			<S.Title isView={isView} status={onSelectVal === '선택해 주세요'}>
				<div>{onSelectVal}</div>
				<span>
					<Down_Icon />
				</span>
			...
			{isView && (
				<S.Box>
					{Data.map(data => (
						<S.List value={data.value} onClick={onClickSelect}>
							{data.text}
						</S.List>
					))}
				</S.Box>
